#import gym
#使用经过处理即包装好的gym游戏环境，位于atari_wrappers.py中
#可以不用考虑一些游戏场景和过程的切换，似乎还能加速画面，便于focus on RL algorithm
...
def main(test=False):
    ...
    if test:
        #num = 0
        num = int(2e7)
        temp = "./save{}.pth".format(num)
        policy_net.load_state_dict(torch.load(temp))
        NUM_STEPS = 10
    else:
        NUM_STEPS = 25000000
    ...
    def evaluate(step, policy_net, device, env, n_actions, eps=0.05, num_episode=5, test=False):
        ...
        for i in range(num_episode):
            ...
            while not done:
                ...
                action, eps = my_action.select_action(state, train)
    ...
    for step in progressive:
        ...
        memory.push(torch.cat(list(q)).unsqueeze(0), action, reward,
                    done)  # here the n_frame means next frame from the previous time step
        episode_len += 1
        ...
        if step % EVALUATE_FREQ == 0 and not test:
            evaluate(step, policy_net, device, env_raw, n_actions, eps=0.05, num_episode=15)
            t = "./save{}.pth".format(step)
            torch.save(policy_net.state_dict(), t)
    env.close()
